...
var Xn = 0.95047, Yn = 1, Zn = 1.08883, t0 = 4 / 29, t1 = 6 / 29, t2 = 3 * t1 * t1, t3 = t1 * t1 * t1, deg2rad$1 = Math.PI / 180, rad2deg = 180 / Math.PI;
function xyz2lab(t) {
...
function getTileJson(glSource, styleUrl, options) {
    ...
    if (!promise || options.transformRequest) {
        ...
        if (url) {
            var normalizedUrl = normalizeSourceUrl(url, options.accessToken, options.accessTokenParam || 'access_token', styleUrl || location.href);
            if (url.startsWith('mapbox://')) {
                ...
                promise = fetchResource('Source', normalizedUrl, options).then(function (tileJson) {
                    for (var i = 0, ii = tileJson.tiles.length; i < ii; ++i) {
                        var tileUrl = tileJson.tiles[i];
                        var normalizedTileUrl = normalizeSourceUrl(tileUrl, options.accessToken, options.accessTokenParam || 'access_token', normalizedUrl || location.href);
        ...
        getGlStyle(glStyle, options).then(function (glStyle) {
            ...
            function assignSource() {
                if (layer instanceof VectorTileLayer) {
                    ...
                    var source = setupGeoJSONSource(glStyle.sources[sourceId], styleUrl, options);
                    var targetSource = layer.getSource();
                    if (!targetSource) {
                        layer.setSource(source);
                    } else if (source !== targetSource) {
...
function setBackground(mapOrLayer, layer) {
    ...
    function updateStyle(resolution) {
        ...
        if (paint['background-color'] !== undefined) {
            ...
            if (element) {
                element.style.background = Color$1.parse(bg).toString();
            }
        }
...
function setupGeoJSONSource(glSource, styleUrl, options) {
    ...
    return new VectorSource(assign({
...
function processStyle(glStyle, map, styleUrl, options) {
    ...
    if (!view.isDef() && !view.getRotation() && !view.getResolutions()) {
        view = new View(assign(view.getProperties(), { maxResolution: defaultResolutions[0] }));
        map.setView(view);
    }
...
function apply(map, style, options) {
    ...
    if (typeof style === 'string') {
        ...
        promise = new Promise(function (resolve, reject) {
            processStyle(style, map, '', options).then(function () {
                resolve(map);
            }).catch(reject);
        });
    }
    ...
    return new Promise(function (resolve, reject) {
        var setStyle = function () {
            ...
            if (source instanceof VectorSource || source instanceof VectorTileSource) {
                applyStyle(layer, glStyle, layerIds, assign({ styleUrl: styleUrl }, options)).then(function () {
